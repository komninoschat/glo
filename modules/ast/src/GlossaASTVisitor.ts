...
interface GlossaASTVisitor {
  visitSwap(node: AST.SwapAST): never;
  visitAlgorithm(node: AST.AlgorithmAST): never;
}

abstract class GlossaASTVisitor<T = unknown> {
  ...
  public visit(node: AST.AST): T {
    if (node instanceof AST.AssignmentAST) {
      ...
      throw new GLOError(
        node,
        `Program error: Unknown or not implemented AST node type on glossa visitor ${node.constructor.name}`,
      );
    }
  }
...
export default GlossaASTVisitor;
